...
try:
    from config import OPENROUTER_API_KEY as _api_key
except ImportError:
    _api_key = os.environ.get('OPENROUTER_API_KEY', '').strip()

if not _api_key:
    raise RuntimeError(
        'OPENROUTER_API_KEY environment variable or config.py is not set or is empty.'
    )
...
def ask_qwen(image_path, prompt, max_new_tokens=1024):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print(f"  [ask_qwen] Timeout hit. Retrying in 5s... (Attempt {attempt+1}/{max_retries})")
                time.sleep(5)
                continue
            raise RuntimeError(
        ...
        if _resp.status_code == 429:
            if attempt < max_retries - 1:
                print(f"  [ask_qwen] Rate limit hit (429). Retrying in 5s... (Attempt {attempt+1}/{max_retries})")
                time.sleep(5)
                continue
...
def run_pipeline_single_rule(image_path, rule, output_dir, cached_views=None):
    """
    Executes the pipeline for a single rule.
