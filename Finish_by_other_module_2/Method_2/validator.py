...
# Ensure we strictly use config.py and not an old cached environment variable
...
def stage_4_arch_trigger_validation(arch_crop_path, rule, arch_ref="NA"):
    ...
    Provide coordinates using keys: {json.dumps(coord_keys)}
    CRITICAL: The 'coordinates' field MUST be a strictly valid nested JSON dictionary containing key-value pairs. For example: {json.dumps(example_coords)}. Do NOT output an array or invalid JSON!
    """
    expected_format = {
        ...
        "coordinates": {
            coord_keys.get("x1", "x1"): 0, coord_keys.get("y1", "y1"): 0,
            coord_keys.get("x2", "x2"): 0, coord_keys.get("y2", "y2"): 0,
        },
        "confidence": 95,
        "reasoning":  "Explanation",
    }
...
def run_pipeline(sub_crop_path, arch_crop_path, rule, output_dir, arch_ref="NA"):
    """
    sub_crop_path  : path to the submittal crop image
    arch_crop_path : path to the arch crop image (or "NA" to skip Stage 4)
    rule           : dictionary containing the rule JSON logic
    output_dir     : where results.json + markup image will be saved
    arch_ref       : architectural reference string (e.g. "7/AE407")
    """
    ...
    markup_image_path = os.path.join(output_dir, f"{rule_base}_{sub_base}_markup.jpg")

    if not stage_2_rule_source_gate(rule):
        print("Architectural Drawing not in required sources. Terminating.")
        return

    view_res = stage_3_view_applicability(sub_crop_path, rule)
    if not view_res.get("view_applicable") or not view_res.get("geometry_applicable"):
        status = rule.get("applicability_logic", {}).get("if_view_not_applicable", "OMITTED")
        print(f"View/Geometry not applicable. Status: {status}")
        stage_8_final_json(rule, view_res, {}, {}, {"status": status},
                           {"markup_required": False}, output_path=output_path)
        return

    if arch_na:
        print("\n--- Stage 4: ARCH Trigger Validation --- [SKIPPED — ARCH REF: NA]")
    ...
    decision_res = stage_6_decision_logic(view_res, arch_res, sub_res, rule)
    markup_res   = stage_7_markup_decision(decision_res, rule)
    stage_8_final_json(rule, view_res, arch_res, sub_res, decision_res, markup_res,
                       output_path=output_path)

    if markup_res.get("markup_required"):
        ...
        try:
            ...
            if img is not None:
                ...
                color      = (0, 255, 255) if markup_res.get("markup_color") == "yellow" else (0, 0, 255)
                text_color = (0, 0, 0)     if markup_res.get("markup_color") == "yellow" else (255, 255, 255)
                text       = f"[{decision_res.get('status','FAIL')}] {markup_res.get('markup_label','MARKUP')}"
                text_size  = cv2.getTextSize(text, font, font_scale, thickness_txt)[0]
                bi   = int(w * 0.10)
                cv2.rectangle(img, (bi, bi), (w - bi, h - bi), color,
                              thickness=max(3, int(w / 150.0)))
                tbx1 = bi
                tby1 = max(bi, bi - text_size[1] - 20)
                tbx2 = tbx1 + text_size[0] + 20
                tby2 = tby1 + text_size[1] + 20
                cv2.rectangle(img, (tbx1, tby1), (tbx2, tby2), color, -1)
                cv2.putText(img, text, (tbx1 + 10, tby2 - 10),
                            font, font_scale, text_color, thickness_txt, cv2.LINE_AA)
                cv2.imwrite(markup_image_path, img)
                print(f"Saved markup image to {markup_image_path}")
                
                # Generate PDF markup
                markup_pdf_path = os.path.join(output_dir, f"{rule_base}_{sub_base}_markup.pdf")
                import fitz
                img_doc = fitz.open(sub_crop_path)
                ...
                # Get colors
                color_rgb = (1.0, 1.0, 0.0) if markup_res.get("markup_color") == "yellow" else (1.0, 0.0, 0.0)
                text_color_rgb = (0.0, 0.0, 0.0) if markup_res.get("markup_color") == "yellow" else (1.0, 1.0, 1.0)
                
                w_pdf, h_pdf = page.rect.width, page.rect.height
                bi_pdf = w_pdf * 0.10
                rect_pdf = fitz.Rect(bi_pdf, bi_pdf, w_pdf - bi_pdf, h_pdf - bi_pdf)
                annot = page.add_rect_annot(rect_pdf)
                annot.set_colors(stroke=color_rgb)
                annot.set_border(width=max(3, int(w_pdf / 150.0)))
                annot.update()
                
                text_rect = fitz.Rect(bi_pdf, max(0, bi_pdf - 40), w_pdf - bi_pdf, bi_pdf)
                t_annot = page.add_freetext_annot(text_rect, text, fontsize=max(12, int(w_pdf/50)), text_color=color_rgb)
                t_annot.update()
                
                doc.save(markup_pdf_path)
                doc.close()
                print(f"Saved PDF markup to {markup_pdf_path}")
                
        except Exception as e:
            print(f"Warning: Failed to create markup image/pdf: {e}")


# ============================================================
def auto_run_all(pdf_basename=None):
    import glob
    print(f"--- Starting Auto-Validation Pipeline {f'for {pdf_basename}' if pdf_basename else ''} ---")
    ...
    # 2. Load Submittal Metadata for lookup
    if pdf_basename:
        meta_files = [f"submittal_metadata_{pdf_basename}.json"]
    else:
        meta_files = glob.glob("submittal_metadata*.json")
        
    if not meta_files or not os.path.exists(meta_files[0]):
        print("No submittal_metadata.json found!")
        return
    meta_file = meta_files[0]
    with open(meta_file, 'r', encoding='utf-8') as f:
        metadata = json.load(f)
        
    # Create lookup map: source_file -> metadata
    crop_to_meta = {}
    for entry in metadata:
        norm_path = os.path.normpath(entry.get("source_file", ""))
        crop_to_meta[norm_path] = {
            "arch_ref": entry.get("arch_ref", ""),
            "view_name": entry.get("view_name", ""),
            "view_type": entry.get("view_type", "")
        }
        
    # 3. Iterate over Submittal Crops
    submittal_base = "Submittal_crop"
    arch_base = "Arch_crop"
    
    if not os.path.exists(submittal_base):
        print(f"Submittal crop directory not found: {submittal_base}")
        return
        
    for pdf_name in os.listdir(submittal_base):
        # If a specific PDF was provided, skip all others
        if pdf_basename and pdf_name != pdf_basename:
            continue
            
        pdf_dir = os.path.join(submittal_base, pdf_name)
        if not os.path.isdir(pdf_dir):
            continue
            
        for sheet_name in os.listdir(pdf_dir):
            sheet_dir = os.path.join(pdf_dir, sheet_name)
            if not os.path.isdir(sheet_dir):
                continue
                
            for crop_img in os.listdir(sheet_dir):
                if not crop_img.lower().endswith((".png", ".jpg", ".jpeg")):
                    continue
                    
                sub_crop_path = os.path.join(sheet_dir, crop_img)
                
                # Fetch metadata
                norm_sub_path = os.path.normpath(sub_crop_path)
                meta_info = crop_to_meta.get(norm_sub_path, {})
                arch_ref = meta_info.get("arch_ref", "")
                
                # Use metadata to build cached_views, bypassing LLM guessing to avoid hallucination/errors
                cached_views = [{
                    "view_id": "V1",
                    "detected_view_type": meta_info.get("view_name", "Unknown"),
                    "view_type_classification": meta_info.get("view_type", "Unknown"),
                    "confidence_score": 100.0,
                    "scope_coordinates": {"x1": 0, "y1": 0, "x2": 9999, "y2": 9999}
                }]
                
                arch_crop_path = "NA"
                if arch_ref and arch_ref != "NA" and "/" in arch_ref:
                    # Construct arch_crop_path
                    parts = arch_ref.split("/")
                    dr_num = parts[0].strip()
                    sh_name = parts[1].strip()
                    # e.g. Arch_crop/AE406/AE406_1.png
                    expected_arch_crop = os.path.join(arch_base, sh_name, f"{sh_name}_{dr_num}.png")
                ...
                for rule in rules_data:
                    print(f"\nProcessing {sub_crop_path} against rule {rule.get('rule_id', 'Unknown')}")
                    ...
                    if requires_arch:
                        run_pipeline(sub_crop_path, arch_crop_path, rule, output_dir, arch_ref)
                    else:
                        from submittal import run_pipeline_single_rule
                        run_pipeline_single_rule(sub_crop_path, rule, output_dir, cached_views=cached_views)
...
if __name__ == "__main__":
    ...
    import sys
    pdf_basename = sys.argv[1] if len(sys.argv) > 1 else None
    auto_run_all(pdf_basename)
