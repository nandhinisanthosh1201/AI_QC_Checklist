...
def save_result(submittal_id: str, view_sheet_id: str, rule: RuleDefinition, result: ViewResult, markup_path: str, view_metadata: Any, arch_view_title: str, output_base_dir: str = "Output") -> None:
    ...
    """
    rule_dir_name = rule.rule_id
    active_code_tag = getattr(rule, "active_code_tag", None)
    if active_code_tag:
        rule_dir_name = f"{rule.rule_id}_{active_code_tag}"
    rule_dir = os.path.join(output_base_dir, submittal_id, view_sheet_id, rule_dir_name)
    ...
    final_output = RuleEvaluationResult(
        ...
        active_code_tag=getattr(rule, "active_code_tag", None),
        active_description=getattr(rule, "active_description", None),
