...
def resize_image(input_path, output_dir, max_size=2000):
    ...
    # Bypass resizing to maintain high quality auto-crop text visibility
    scale_x = scale_y = 1.0
    print(' -> no resize needed (auto-crop mode)')
...
def ask_qwen(image_path, prompt, max_new_tokens=4096):
    ...
    # ── 3. Call OpenRouter API with Retry on 429 ──────────────────────────────
    import time
    max_retries = 5
    for attempt in range(max_retries):
        ...
        if _resp.status_code == 429:
            if attempt < max_retries - 1:
                ...
                print(f'[ask_qwen] OpenRouter returned HTTP 429 (Rate Limited). Retrying in {wait_time}s... (Attempt {attempt+1}/{max_retries})')
                time.sleep(wait_time)
                continue
            else:
                raise RuntimeError(
                    f'[ask_qwen] OpenRouter API returned HTTP 429 after {max_retries} retries.\n'
        ...
        # If we got 200 OK, break out of the retry loop
        break
...
def build_applicability_prompt(rule, img_w, img_h, cached_views=None, use_cached_hints=False):
    ...
    lines += [
        ...
        '5. geometry_coordinates: If the physical geometry exists, provide the precise pixel bounding box of that geometry (e.g. the equipment, blue dimension line, etc).',
        '6. DO NOT search for note text here. Geometry check only.',
        '7. Set status=OMITTED when view scope does not match OR geometry is absent.',
        '8. Set status=REVIEW_REQUIRED only when geometry/view match is genuinely unclear.',
        '9. Do NOT set geometry_applicable=true only because the detected view type matches VIEWS TO CHECK.',
        '10. Set geometry_applicable=true ONLY when the required visible geometry condition is clearly present in that same view.',
        '',
        _JSON_OUTPUT_RULES,
        '',
        ...
        '    "geometry_coordinates": {"x1": 0, "y1": 0, "x2": 0, "y2": 0}',
        '  }]',
        '}',
    ]
    return '\n'.join(lines)
...
def build_note_prompt(rule, applicable_views, img_w, img_h):
    ...
    lines = [
        ...
        '',
        _JSON_OUTPUT_RULES,
        '  - detected_text field: You MUST write the EXACT, FULL text exactly as it appears on the drawing (e.g. write "CENTERLINE OF IN-WALL STRAPPING (STRAPPING BY OTHERS)" if that is what you see).',
        '  - DO NOT chop off prefixes or suffixes to force a match with the required variants! If you chop words to force a match, the system will break.',
        '',
        'Return ONLY valid JSON (no markdown):',
        '{',
        f'  "rule_id": "{rule.get("rule_id")}",',
...
def post_process_note_view(nv, rule, app_view=None):
    ...
    if not variant_matched:
        ...
        # Always clear detected_text and coordinates so the output never shows the wrong note.
        for ent in entities:
            if ent.get('detected_text') and ent['detected_text'].upper() not in ('NOT FOUND', 'NULL', 'NONE', ''):
                ...
                ent['coordinates'] = {'x1': 0, 'y1': 0, 'x2': 0, 'y2': 0}
...
def combine_stage_results(rule, app_result, note_result, scale_x=1.0, scale_y=1.0):
    ...
    for idx, av in enumerate(app_views):
        ...
        geom_coords_raw = av.get('geometry_coordinates', {'x1': 0, 'y1': 0, 'x2': 0, 'y2': 0})
        geom_coords     = unscale(geom_coords_raw)
...
def markup_results(original_image_path, results, output_path):
    ...
    for rule_idx, rr in enumerate(results):
        ...
        for view in rr.get('view_results', []):
            ...
            if not drew:
                geom = view.get('geometry_coordinates', {})
                if valid_box(geom):
                    geom = clamp_box(geom, W, H)
                    if valid_box(geom):
                        x1, y1, x2, y2 = get_box_ints(geom)
...
def draw_markup_box_pdf(page, x1, y1, x2, y2, label, status):
    import fitz
    bgr = STATUS_COLORS.get(status, (0, 0, 255))
    color = (bgr[2]/255.0, bgr[1]/255.0, bgr[0]/255.0) # RGB
    rect = fitz.Rect(max(0, x1), max(0, y1), min(page.rect.width, x2), min(page.rect.height, y2))
    annot = page.add_rect_annot(rect)
    annot.set_colors(stroke=color)
    annot.set_border(width=3)
    annot.update()
    
    text = f"[{status}] {label[:55]}"
    
    # Ensure text_rect always has a height of at least 30
    text_y1 = y1 - 30
    text_y2 = y1
    if text_y1 < 0:
        text_y1 = 0
    ...
    t_annot = page.add_freetext_annot(text_rect, text, fontsize=12, text_color=color)
    t_annot.update()
    return True

def markup_results_pdf(original_image_path, results, output_path):
    import fitz
    img_doc = fitz.open(str(original_image_path))
    ...
    print(f'  Markup PDF on original: {W}x{H}')
        ...
        for view in rr.get('view_results', []):
            ...
            for ent in view.get('detected_entities', []):
                ...
                if valid_box(box):
                    ...
                    if valid_box(box):
                        x1, y1, x2, y2 = get_box_ints(box)
                        drew = draw_markup_box_pdf(page, x1, y1, x2, y2, label, vstatus) or drew
            if not drew:
                scope = view.get('scope_coordinates', {})
                if valid_box(scope):
                    scope = clamp_box(scope, W, H)
                    if valid_box(scope):
                        x1, y1, x2, y2 = get_box_ints(scope)
                        drew = draw_markup_box_pdf(page, x1, y1, x2, y2, label, vstatus)
            if not drew:
                # Use a dummy img object just to get dimensions for default_fail_box
                class DummyImg: shape = (H, W)
                x1, y1, x2, y2 = default_fail_box(DummyImg(), rule_idx)
                draw_markup_box_pdf(page, x1, y1, x2, y2, label, vstatus)
                print(f'  [FALLBACK BOX PDF] {label[:40]} — no valid coords')
                
    doc.save(str(output_path))
    doc.close()
    print(f'  ✅ Markup PDF saved: {output_path}')
    return output_path

print('✅ markup_results and PDF markup ready')
...
def run_pipeline_single_rule(image_path, rule, output_dir, cached_views=None):
    """
    Executes the pipeline for a single rule, skipping global view detection if cached_views is provided.
    Outputs a single JSON file formatted as {rule_id}_{stem}_result.json and markup as {rule_id}_{stem}_markup.jpg
    """
    stem = Path(str(image_path)).stem
    ...
    output_json = os.path.join(output_dir, f"{rule_id}_{stem}_result.json")
    output_markup = os.path.join(output_dir, f"{rule_id}_{stem}_markup.jpg")
    
    print(f'\n{"="*60}\n[{stem[:40]}] {rule_id} (Single Rule Mode)')
    
    use_cached_hints = False
    if cached_views is not None:
        use_cached_hints = is_cached_views_reliable(cached_views, img_w, img_h)
        print(f'[GLOBAL VIEW DETECTION] Using provided cache: reliable={use_cached_hints}')
    ...
    try:
        ...
        print(f'[APP ERROR] {e}')
        final = {
            'rule_id': rule_id, 'rule_name': rule.get('rule_name'),
            'sheet_status': 'REVIEW_REQUIRED', 'view_results': [],
            'markup_required': False, 'markup_color': rule.get('markup_color', 'red'),
            'markup_label': rule.get('markup_label', 'CHECK'),
            'severity': rule.get('severity', 'Major'), 'priority': rule.get('priority', 'High')
    ...
    if final.get("markup_required"):
        # submittal markup takes a list of results, we wrap it in a list
        markup_results(image_path, [final], output_markup)
        
        # generate PDF markup
        output_markup_pdf = os.path.join(output_dir, f"{rule_id}_{stem}_markup.pdf")
        markup_results_pdf(image_path, [final], output_markup_pdf)
...
if __name__ == '__main__':
    ...
    RULE_JSON_PATH = r'C:\Finish_byother\rules\inwall.json'
