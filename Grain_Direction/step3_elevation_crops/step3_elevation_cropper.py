...
def find_border_and_divider(page, matrices=None):
    ...
    # [HOTFIX] Bypass border if it's crazily small (like 6 points width for Docusign)
    if right - left < rect.width * 0.5:
        left = 0.0
        top = 0.0
        right = rect.width
        bottom = rect.height
...
def _generic_blocks_to_anchors(title_blocks, method):
    ...
    There's no ARCH REF concept in this generic mode, so we store the
    detected drawing number there instead (still useful in the manifest).
    """
    anchors = []
        ...
        anchors.append(
            {
                ...
                "arch_ref": (number_span or {}).get("text", "").strip(),
                "x0": x0,
                "x1": x1,
                "y0": y0,
                "y1": y1,
...
def make_long_path_safe(p):
    s = str(os.path.abspath(p))
    if os.name == "nt" and not s.startswith("\\\\?\\"):
        return "\\\\?\\" + s
    return s


def crop_and_save_boxes(page, boxes, crops_dir, pno, zoom=2.5):
    """
    Render and save one clean PNG per box, named after its detected title,
    into `crops_dir`. Must be called BEFORE draw_boxes_on_page() mutates
    the page with the red rectangles/labels, so these crops come out
    clean (no annotation overlay) -- the separate full-page
    "p###_bboxes.png" overview is the only place the boxes themselves get
    drawn.

    `boxes[i]["rect"]` is in DISPLAY space, which is the same coordinate
    system page.rect (and therefore get_pixmap()'s `clip` argument) uses,
    so no rotation matrix conversion is needed here -- unlike
    draw_boxes_on_page(), which writes into the RAW content stream and
    does need one.

    Returns a list of saved filenames, one per box, in the same order as
    `boxes` (so callers can zip() them back onto each box/manifest row).
    Filenames collide-check within crops_dir (two drawings on the same
    page whose titles slugify to the same string get a `_1`, `_2`, ...
    ...
    for b in boxes:
        ...
        fname = f"p{pno:03d}_{slug}.png"
        fpath = crops_dir / fname
        n = 1
        while fpath.exists():
            fname = f"p{pno:03d}_{slug}_{n}.png"
            fpath = crops_dir / fname
            n += 1

        pix.save(make_long_path_safe(fpath))
        names.append(fname)
    return names
...
def process_pdf(pdf_path, out_dir, zoom=2.5, pages=None, pad=4.0, margin=3.0, min_gap=30.0,
                 use_vlm=False, label=True, filter_str=None):
    doc = fitz.open(pdf_path)
    
    # Automatically create a subfolder using the PDF's filename (without extension)
    # Avoid duplicating if out_dir already ends with pdf_name
    pdf_name = Path(pdf_path).stem
    if Path(out_dir).name != pdf_name:
        out_dir = Path(out_dir) / pdf_name
    out_dir.mkdir(parents=True, exist_ok=True)

    # Folder that holds the actual per-drawing crops, named after their
    # detected titles (see crop_and_save_boxes() / PATCH NOTES above).
    ...
    for pno in page_indices:
        ...
        print(f"[DEBUG p{pno:03d}] border: L={left} T={top} R={right} B={bottom} div={divider}")
        ...
        for b in boxes:
            ...
            if r.is_empty or r.width <= 1 or r.height <= 1:
                print(f"[DEBUG p{pno:03d}] Dropping invalid box: {b['title']} rect={b['rect']}")
            else:
                valid_boxes.append(b)
        boxes = valid_boxes
        
        if not boxes:
            print(f"[DEBUG p{pno:03d}] No valid boxes remaining after rect check.")
            continue
        ...
        # Save a clean, cropped PNG per drawing (named after its title)
        # BEFORE the boxes get drawn onto the page below.
        crop_names = crop_and_save_boxes(page, boxes, crops_dir, pno, zoom=zoom)
    ...
    with open(manifest_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f, fieldnames=["page", "file", "crop_file", "title", "scale", "arch_ref", "method", "x0", "y0", "x1", "y1"]
        )
        writer.writeheader()
...
def main():
    ...
    ap.add_argument("out_dir", help="Directory to write annotated page PNGs + manifest.csv into")
