...
VIEW-AWARE CROP BOXES
  ...
  FIX 3 (duplicate detections): DINO's multi-phrase prompt frequently
  produces 2-3 boxes for the SAME drawing (different phrases in the prompt
  each best-matching that drawing, e.g. "architectural section" and
  "section building struc" both firing on one elevation). Their IoU is
  often too low for tiled_nms's IoU-based merge (a loosely-fit box vs a
  tightly-fit box on the same object commonly sits below the nms_iou
  threshold), so duplicates survived quality_filter and were assigned to
  the same view multiple times, or double-counted as separate spurious
  hits in anchor scoring. Fixed by running a containment-based merge
  (dedupe_dets, reusing _merge_contained) on dets right after
  quality_filter, before view assignment.
...
D = dict(
    ...
    # Duplicate-detection merge (same drawing, multiple prompt phrases)
    dedupe_contain_thr=0.6,   # smaller box >= this fraction inside larger -> merge
...
def quality_filter(dets, img_w, img_h, cfg, log):
    ...
    # In tiled mode, filter whole-tile hallucinations: boxes that span nearly
    # the full tile in both dimensions are DINO firing on the tile border,
    # not a real drawing. Threshold: >= 88% of tile_px in BOTH W and H.
    tile_px = float(cfg.get("tile", 2800))
    tile_frac = float(cfg.get("max_tile_det_frac", 0.88))
    tile_limit = tile_px * tile_frac
    keep = []
    ...
    for d in dets:
        ...
        # Whole-tile hallucination: large box covering most of one tile in both
        # dimensions. Only applies in tiled mode (when tile_limit makes sense).
        if w >= tile_limit and h >= tile_limit:
            log.append(f"drop whole-tile {w:.0f}x{h:.0f}")
            continue
...
def score_against_anchors(clusters_pdf, anchors, up_tol=120.0, side_tol=40.0):
    claims = []
...
def assign_boxes_to_views(boxes, views, cfg, ytops=None, global_tops=None):
    """
    ...
    global_tops[vi], if given, is the reading-order top boundary for view
    vi — the bottom of the LAST view that appears above vi in page reading
    order, regardless of horizontal overlap. This is a hard exclusion line:
    a DINO box whose bottom sits above global_tops[vi] cannot belong to vi
    even if the gap test would otherwise pass. This prevents notes/schedules
    boxes that appear in the same vertical band as an earlier view (but in a
    different horizontal column) from being falsely assigned to a later view.
    """
    pad = cfg["view_pad"]
    assign = []
    ...
    for b in boxes:
        ...
        for vi, v in enumerate(views):
            ...
            # Hard upper boundary: box must not sit entirely above the
            # reading-order top of this view (content from an earlier row
            # on a different horizontal column must not bleed into this view).
            if global_tops is not None and b[3] < global_tops[vi]:
                continue

            room = (title_y - ytops[vi]) if (ytops is not None and ytops[vi] > 0) else 0.0
...
def _merge_contained(items, thr=0.8):
    ...
    A size-ratio guard prevents a large box (e.g. a whole-tile hallucination
    that survived quality_filter) from absorbing many small real detections.
    Two boxes are only considered for containment-merge when their areas are
    within a 1:3 ratio of each other (size_ratio >= 0.35).
    """
    items = list(items)
    changed = True
    ...
    while changed:
        ...
        for i in range(len(items)):
            for j in range(i + 1, len(items)):
                ...
                ar_a, ar_b = area(a), area(b)

                if ar_a == 0 or ar_b == 0:
                    continue

                # Only merge boxes of comparable size (within ~1:3 area ratio)
                if min(ar_a, ar_b) / max(ar_a, ar_b) < 0.35:
                    continue
...
def dedupe_dets(dets, thr=D["dedupe_contain_thr"]):
    """
    Collapse near-duplicate detections of the same drawing.

    Grounding DINO's multi-phrase prompt commonly produces 2-3 boxes for
    ONE physical drawing, each best-matching a different phrase in the
    prompt (e.g. "architectural section" and "section building struc"
    both firing on the same elevation). These boxes are close but rarely
    identical, so their IoU often falls below tiled_nms's iou_thresh and
    they survive as separate detections -> duplicate/overlapping crops
    and inflated "spurious" counts in anchor scoring.

    Containment (is the smaller box mostly INSIDE the larger one) is a
    better test here than IoU, since one query's box is often tighter
    than another's even on the exact same object. Reuses the same merge
    logic already used for unassigned boxes (_merge_contained), applied
    to the WHOLE detection set before view assignment.
    """
    items = [
        {"box": d["box"], "score": d["score"], "label": d["label"]}
        for d in dets
    ]
    merged = _merge_contained(items, thr=thr)
    return [
        {"box": m["box"], "score": m["score"], "label": m["label"]}
        for m in merged
    ]
...
def compute_crops(page, dets, anchors, cfg, sf):
    ...
    # FIX 4 (row-aware global reading-order tops):
    # Group views into rows by title line y (within row_tol).
    # Views in the SAME row are side-by-side — they share the same row top boundary
    # (the bottom of the row above them, or 0.0 if top row).
    # This prevents views in the same row from falsely rejecting each other's DINO boxes.
    views_by_row = []
    row_tol = 40.0
    for vi, v in enumerate(views):
        ly = v["line"][2]
        placed = False
        for r in views_by_row:
            if abs(views[r[0]]["line"][2] - ly) <= row_tol:
                r.append(vi)
                placed = True
                break
        if not placed:
            views_by_row.append([vi])

    views_by_row.sort(key=lambda r: sum(views[i]["line"][2] for i in r) / len(r))
    global_tops = [0.0] * len(views)
    running_bottom = 0.0
    for r in views_by_row:
        for vi in r:
            global_tops[vi] = running_bottom
        row_bottom = max(views[vi]["bottom"] for vi in r)
        running_bottom = max(running_bottom, row_bottom)

    boxes = [[c / sf for c in d["box"]] for d in dets]
    assign = assign_boxes_to_views(boxes, views, cfg, ytops, global_tops)
    ...
    if not cfg.get("crop_drop_unanchored", False):
        for it in sorted(
            ...
            # Suppress unanchored crops that heavily overlap existing view/fallback crops
            ibox = it["box"]
            ibox_area = max(1e-6, (ibox[2] - ibox[0]) * (ibox[3] - ibox[1]))
...
def run_one(
    ...
    # FIX 3: collapse near-duplicate detections of the same drawing
    # (multiple prompt phrases matching one object) before assignment.
    n_before_dedupe = len(dets)
    dets = dedupe_dets(dets, thr=cfg.get("dedupe_contain_thr", D["dedupe_contain_thr"]))

    if len(dets) < n_before_dedupe:
        log.append(f"deduped {n_before_dedupe} -> {len(dets)} boxes")
    ...
    if (
        ...
        try:
            ...
            n_before_dedupe_t = len(dets_t)
            dets_t = dedupe_dets(
                dets_t, thr=cfg_t.get("dedupe_contain_thr", D["dedupe_contain_thr"])
            )

            if len(dets_t) < n_before_dedupe_t:
                log_t.append(f"deduped {n_before_dedupe_t} -> {len(dets_t)} boxes")
...
def main():
    ...
    parser.add_argument("--dedupe-contain-thr", type=float,
                        default=D["dedupe_contain_thr"],
                        help="Containment fraction above which two boxes are "
                             "treated as duplicates of the same drawing "
                             "(default 0.6)")
    parser.add_argument("--no-dedupe", action="store_true",
                        help="Disable the duplicate-detection merge step")
    ...
    print(f"dedupe     : {'off' if args.no_dedupe else args.dedupe_contain_thr}")
    print(f"pages      : {pages}")
    print(f"modes      : {modes}")
    print("=" * 72)
    ...
    for page_no in pages:
        ...
        for mode in modes:
            cfg = {
                ...
                "dedupe_contain_thr": (
                    None if args.no_dedupe else args.dedupe_contain_thr
                ),
